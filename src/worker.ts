...
const extension = defineExtension({
  manifest: {
    ...
    id: 'org.neoanki.card-timer', name: 'Card Timer', version: '2.0.1', publisher: 'NeoAnki contributors',
  ...
  async handle(request, host) {
    if (request.type !== 'command') return { type: 'error', requestId: request.type === 'planning-signals' ? request.request.requestId : request.type === 'cancel' ? request.operationId : request.requestId, code: 'unsupported', message: 'Card Timer cannot handle this request.' }
